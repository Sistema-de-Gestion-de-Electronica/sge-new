...
type FallasData = RouterOutputs["fallas"]["getAllFallas"]["reportes"][number];
      ...
      cell: ({ row }) => {
        return (
          <div className="max-w-xs">
            <span className="text-sm">{row.original.reportadoPor.apellido + " " + row.original.reportadoPor.nombre}</span>
          </div>
        );
      },
    }),
...
export const getColumnasFallasNames = () => {
  return ["Laboratorio", "Equipo", "Marca", "Modelo", "Fallas", "Descripción Falla", "Reportado Por", "Fecha Reporte", "Asignado A", "Estado"];
};
