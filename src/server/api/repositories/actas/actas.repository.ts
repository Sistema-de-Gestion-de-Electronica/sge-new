...
export const getAllActas = async (ctx: { db: PrismaClient }, input: InputGetAll, esConsejero: boolean, esAdmin: boolean, verAbierta: boolean) => {

  const where: Prisma.ActaWhereInput = {
    ...(esAdmin ? {} : { visibilidad: "VISIBLE" }),
    ...((esConsejero || esAdmin) && verAbierta ? {} : { estado: "CERRADA" }),
  };
