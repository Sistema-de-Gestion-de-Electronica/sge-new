...
export const FallasFilterReportadoPor = ({ filters }: Props) => {
  const { reportadoPor, onReportadoPorChange } = useFallasQueryParam(filters);

  const [query, setQuery] = useState("");

  const { data, isLoading, isError } = api.usuariosFallas.getUsuariosConPermisoReportarFallas.useQuery();
  ...
  const currentUsuario = useMemo(() => {
    ...
    return usuarios.find((item) => String(item.id) === reportadoPor);
  }, [usuarios, reportadoPor]);
  ...
  return (
    <div className="w-full">
      <Autocomplete
        ...
        onChange={(value) => {
          onReportadoPorChange(value?.id ? String(value.id) : "");
          setQuery("");
        }}
      />
    </div>
